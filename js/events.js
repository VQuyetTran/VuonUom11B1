...
function openPointModal(studentId) {
  if (!isTeacherMode) { toast("⚠️ Bạn cần đăng nhập với quyền giáo viên."); return; }
...
function openAddStudentModal() {
  if (!isTeacherMode) { toast("⚠️ Bạn cần đăng nhập với quyền giáo viên."); return; }
  const teamSelect = document.getElementById("newStudentTeam");
...
function confirmAddStudent() {
  ...
  const team = Number(document.getElementById("newStudentTeam").value);
  if (addNewStudent(name, team)) {
